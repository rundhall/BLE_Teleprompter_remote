...
//General IO//
#define BUTTON_1_D14 14  //Button 1 MEDIA_PLAY_PAUSE start, stop teleporter
#define BUTTON_2_D18 18 // Button 2 VOLUME_MUTE Scroll up
#define BUTTON_3_D19 19 // Button 3 VOLUME_UP Scroll down
#define BUTTON_4_D21 21 // Button 4 VOLUME_DOWN Increase speed
#define BUTTON_5_D23 23 // Button 5 ESCAPE Decrease speed
...
BleKeyboard bleKeyboard("BLE_Teleporter", "Rundhall", 100);
...
void recvWithEndMarker1() {
  static byte ndx = 0;
  char rc;
  while (Serial.available() > 0 && newData == false) {    //If there is new data and package is handeled
    rc = Serial.read();
    //Serial.print(rc);
    if (rc != endMarker) {    //If reads other then end marker it stores in a buffer else a new command has arrived. 
